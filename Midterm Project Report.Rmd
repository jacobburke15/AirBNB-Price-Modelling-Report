...
Firstly, we must read in our data for each of the 10 cities. 
...
We see that for all cities, the overall range of accomodation numbers for a listing spans from 1-10 persons. The most common accomodations numbers amongst all city listings are 1-2 OR 3-4 people. 
...
fit <- lmer(log(price) ~ reviews + overall_satisfaction + bedrooms + accommodates + room_type + (1 + bedrooms + accommodates| City), data = BNBdata)
...
We can also see this illustrated in the binned residual plot, as the majority of residuals are found within the proper interval, but there are certainly a number of outliers outside of that interval as well. 
...
ggplot(BNBdata) + geom_density(aes(x = log(BNBdata$price)), color = 'red') + geom_density(aes(x = log(predict_values$predict_new)), color = 'blue') + facet_wrap(~City)
